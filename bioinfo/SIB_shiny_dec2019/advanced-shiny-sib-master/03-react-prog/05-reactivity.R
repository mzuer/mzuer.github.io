...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      # add here a button for saving the file
      actionButton("saveFile", "Save table to file")
...
server <- function(input, output) {
  ...
  # => will save to file everytime input$genere changes !
  # => change it to do this only when button is pressed
  # observe({
  ...
  # => change here, save only when the button is clicked
  observeEvent(input$saveFile, {
    cat(paste0(as.character(input$saveFile), "\n")) # NB: start at 0 and will be incremented everytime you click on
    filename <- paste0("movies-subset", format(Sys.time(), "%Y-%m-%d-%H-%M-%S"), ".csv")
    write_csv(movies_subset(), path = paste0("saved-data/", filename))
  })
